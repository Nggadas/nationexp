...
<?php
	include("check.php");
	include("invoice_info_check.php");
?>
<html lang="en">
    <head>
		<meta charset="utf-8">
		...
		<title>View Invoice Details - NationExpress24 Delivery</title>
                        ...
                        <table class=" display table table-striped table-bordered table-hover" id="recent_orders_table" >
                            ...
                            <tbody>
                                <?php
                                    ...
                                    // get booking_no to use on datatable
                                    $sql_booking_no = mysqli_query($connect,"SELECT DISTINCT `booking_no` FROM `invoices` WHERE `invoice_no`='$invoice_no' ORDER BY id DESC");
                                    $inv_num = mysqli_num_rows($sql_booking_no);

                                    if($inv_num > 0) {

                                        while($inv_data = $sql_booking_no->fetch_assoc()) {
                                            $booking_no = $inv_data['booking_no'];

                                            // Get orders placed by user
                                            $sql = mysqli_query($connect,"SELECT * FROM `parcel_details` WHERE `booking_no`='$booking_no' ORDER BY id DESC");
