<?php
	...
	$sqlbnot=mysqli_query($connect,"SELECT COUNT(DISTINCT booking_no) AS delivered FROM `tracking_details` WHERE `account_id`!='' AND `tstatus`='delivered' AND `old`=''");
	while($rowbnot=mysqli_fetch_object($sqlbnot)){ $delivered=$rowbnot->delivered; }

	//Count all Orders in "UnDelivered" status
	$sqlundelivered=mysqli_query($connect,"SELECT COUNT(DISTINCT booking_no) AS undelivered FROM `tracking_details` WHERE `account_id`!='' AND `tstatus`='undelivered' AND `old`=''");
	...
	$sqlorderbooked=mysqli_query($connect,"SELECT COUNT(DISTINCT booking_no) AS orderbooked FROM `tracking_details` WHERE `account_id`!='' AND `tstatus`='order_booked' AND `old`=''");
	...
	$sqlintransit=mysqli_query($connect,"SELECT COUNT(DISTINCT booking_no) AS intransit FROM `tracking_details` WHERE `account_id`!='' AND `tstatus`='in_transit' AND `old`=''");
	...
	$sqloutfordelivery=mysqli_query($connect,"SELECT COUNT(DISTINCT booking_no) AS outfordelivery FROM `tracking_details` WHERE `account_id`!='' AND `tstatus`='out_for_delivery' AND `old`=''");
	...
	$sqlordercancelled=mysqli_query($connect,"SELECT COUNT(DISTINCT booking_no) AS ordercancelled FROM `tracking_details` WHERE `account_id`!='' AND `tstatus`='order_cancelled' AND `old`=''");
	while($rowordercancelled=mysqli_fetch_object($sqlordercancelled)){ $ordercancelled=$rowordercancelled->ordercancelled; }

	//Count All customers
	$sqlall=mysqli_query($connect,"SELECT COUNT(email) AS total FROM `register` WHERE `email` !='' AND `account_id`!='' AND `status`='Enabled'");
	while($all=mysqli_fetch_object($sqlall)){ $allcustomers=$all->total; }
