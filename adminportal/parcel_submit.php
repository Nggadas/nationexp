<?php
	...
	if($myno !="") {
		...
		$sql_invoice = mysqli_query("SELECT * FROM `delivery_details` WHERE `booking_no`='$myno' ORDER BY id DESC LIMIT 1");
		...
		$sql_register= mysqli_query("SELECT * FROM `register` WHERE `account_id`='$myid_exists' ORDER BY id DESC LIMIT 1");
		...
		$query_check4 = mysqli_query("SELECT * FROM `settings_delivery_type` WHERE `status` = 'Active' ORDER BY id DESC");
	...
	if(isset($_POST['submit_button'])){
		...
		$sql_email = mysqli_query("SELECT * FROM `register` WHERE `email` != '' AND `account_id`='$acct_id' ORDER BY ID DESC LIMIT 1");
		...
		elseif($myaccount_id_email){
			...
			if($qty=='' || $acct_id=='' || $weight=='' || $payment_method=='' || $delivery_cost == '' || $description == '') {
				...
				$sql_pk1 = mysqli_query("SELECT * FROM `pickup_details` WHERE `booking_no` = '$booking_no' AND `account_id`='$myaccount_id_email' ORDER BY ID DESC LIMIT 1");
				...
				$sql_parcel = mysqli_query("SELECT * FROM `parcel_details` WHERE `booking_no` = '$booking_no' AND `account_id`='$myaccount_id_email' ORDER BY ID DESC LIMIT 1");
				...
				$sql_pay = mysqli_query("SELECT * FROM `payment_details` WHERE `booking_no` = '$booking_no' AND `account_id`='$myaccount_id_email' ORDER BY ID DESC LIMIT 1");
				...
				$rsN = mysqli_query($sqln) or die("Errorn : ".mysqli_error());
				...
				$rsU = mysqli_query($sqlu) or die("Erroru : ".mysqli_error());
