<?php
	...
	if($status){
	$sql_tr_orders = mysqli_query($connect, "SELECT * FROM `tracking_details` WHERE `account_id`='$account_id' AND `status`='$status' AND `old`='' ORDER BY id DESC");
	$row_tr_orders = mysqli_num_rows($sql_tr_orders);
	$val_tr_orders = mysqli_fetch_assoc($sql_tr_orders);
	}
	elseif(!$status){
	$sql_tr_orders = mysqli_query($connect, "SELECT * FROM `tracking_details` WHERE `account_id`='$account_id' AND `status`!='' AND `old`='' ORDER BY id DESC");
	$row_tr_orders = mysqli_num_rows($sql_tr_orders);
	$val_tr_orders = mysqli_fetch_assoc($sql_tr_orders);
	}
